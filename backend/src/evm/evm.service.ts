...
export class EVMService implements OnModuleInit {
    ...
    private initializeClients() {
        for (const [key, chain] of Object.entries(SUPPORTED_CHAINS)) {
            ...
            if (rpcUrls.length === 0) {
                this.logger.warn(`No RPC URLs configured for chain ${chain.name} (${chainId})`);
                continue;
            }
    ...
    async sendUSDCTransaction(
        ...
        const walletClient = createWalletClient({
            account,
            chain: SUPPORTED_CHAINS[this.getChainNameFromId(chainId) as keyof typeof SUPPORTED_CHAINS],
