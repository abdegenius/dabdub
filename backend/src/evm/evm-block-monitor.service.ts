...
import { EVMTransactionEntity, EVMTransactionStatus } from '../database/entities/evm-transaction.entity';
import { SUPPORTED_CHAINS, USDC_CONTRACT_ADDRESSES, CHAIN_IDS, ERC20_ABI } from './evm.constants';
...
export class EVMBlockMonitorService {
    ...
    async monitorBlocks() {
        ...
        try {
            ...
            this.logger.error(`Error in block monitoring: ${error.message}`, error.stack);
        } finally {
    ...
    private async processChain(chainId: number) {
        try {
            ...
            const processUpTo = lastBlock + 10n > currentBlock ? currentBlock : lastBlock + 10n;

            if (processUpTo <= lastBlock) {
                return;
            }

            this.logger.debug(`Processing blocks ${lastBlock + 1n} to ${processUpTo} on chain ${chainId}`);
            ...
            const monitoredAddresses = new Set(wallets.map(w => w.address.toLowerCase()));
    ...
    private async handleDeposit(
        ...
        this.logger.log(`Detected deposit of ${value} units to ${to} on chain ${chainId}`);
